...
//        iu.getQuentity();
    ...
    public DataModel<ItemUnit> getItemsIns() {
        if (institution != null) {
            return new ListDataModel<ItemUnit>(getFacade().findBySQL("SELECT i From ItemUnit i WHERE i.retired=false AND i.institution.id = " + institution.getId()));
        } else {
            return null;
        }
    }
    ...
    public DataModel<ItemUnit> getItemsUni() {
        if (unit != null) {
            return new ListDataModel<ItemUnit>(getFacade().findBySQL("SELECT i From ItemUnit i WHERE i.retired=false AND i.unit.id = " + unit.getId()));
        } else {
            return null;
        }
    }
