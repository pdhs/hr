...
public final class GnController  implements Serializable {
    ...
    List<GnArea> lstItems;
    private GnArea current;
    private DataModel<GnArea> items = null;
    private DataModel<PhmArea> phmAreas;
    ...
    public List<GnArea> getLstItems() {
        return getFacade().findBySQL("Select d From GnArea d");
    }

    public void setLstItems(List<GnArea> lstItems) {
    ...
    public void setCurrent(GnArea current) {
        this.current = current;
    }

    private GnFacade getFacade() {
        return ejbFacade;
    }

    public DataModel<GnArea> getItems() {
        ...
        if (items == null) {
            if (selectText.equals("")) {
                ...
                if (items.getRowCount() > 0) {
                    items.setRowIndex(0);
                    current = (GnArea) items.getRowData();
    ...
    public GnArea searchItem(String itemName, boolean createNewIfNotPresent) {
        GnArea searchedItem = null;
        items = new ListDataModel(getFacade().findAll("name", itemName, true));
        if (items.getRowCount() > 0) {
            items.setRowIndex(0);
            searchedItem = (GnArea) items.getRowData();
        } else if (createNewIfNotPresent) {
            searchedItem = new GnArea();
    ...
    public GnFacade getEjbFacade() {
        return ejbFacade;
    }

    public void setEjbFacade(GnFacade ejbFacade) {
    ...
    public static class GnControllerConverter implements Converter {
        ...
        public Object getAsObject(FacesContext facesContext, UIComponent component, String value) {
            ...
            GnController controller = (GnController) facesContext.getApplication().getELResolver().
                    getValue(facesContext.getELContext(), null, "gnController");
